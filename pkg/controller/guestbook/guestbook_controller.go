...
import (
	...
	informer "my.domain/guestbook/pkg/client/informers/externalversions/apps/v1alpha1"
	lister "my.domain/guestbook/pkg/client/listers/apps/v1alpha1"
	"time"
)

type Controller struct {
	GuestBookSynced cache.InformerSynced
	coreAPIClient   clientset.Interface
	GuestBookLister lister.GuestBookLister
	queue           workqueue.RateLimitingInterface

	syncHandler func(ctx context.Context, key string) error
}

func NewGuestBookController(jsInformer informer.GuestBookInformer, coreAPIClient clientset.Interface) *Controller {
	c := &Controller{
		coreAPIClient:   coreAPIClient,
		GuestBookLister: jsInformer.Lister(),
		GuestBookSynced: jsInformer.Informer().HasSynced,

		queue: workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "GuestBook"),
	}
...
func (c *Controller) Run(ctx context.Context, workers int) {
	...
	if !cache.WaitForCacheSync(ctx.Done(), c.GuestBookSynced) {
...
func (c *Controller) sync(ctx context.Context, key string) (err error) {
	...
	js, err := c.GuestBookLister.GuestBooks(namespace).Get(key)
	...
	d := apps.Deployment{
		...
		Spec: apps.DeploymentSpec{
			Replicas: pointer.Int32Ptr(int32(js.Spec.InstanceAmount)),

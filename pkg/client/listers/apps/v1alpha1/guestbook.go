...
type GuestBookLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.GuestBook, err error)
...
// List lists all GuestBooks in the indexer.
func (s *guestBookLister) List(selector labels.Selector) (ret []*v1alpha1.GuestBook, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.GuestBook))
	})
	return ret, err
}
...
type GuestBookNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.GuestBook, error)
...
// List lists all GuestBooks in the indexer for a given namespace.
func (s guestBookNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.GuestBook, err error) {
...
// Get retrieves the GuestBook from the indexer for a given namespace and name.
func (s guestBookNamespaceLister) Get(name string) (*v1alpha1.GuestBook, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("guestbook"), name)
	}
	return obj.(*v1alpha1.GuestBook), nil
}
